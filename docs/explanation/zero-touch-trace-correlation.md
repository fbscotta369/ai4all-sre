...
## 5. Tier-1 Extension: Alert-to-Trace Correlation
The latest industrial upgrade introduces **Alert-to-Trace** reactive correlation. Instead of just linking Logs to Traces, the system now links **Prometheus Alerts** directly to the relevant **Tempo Spans**.

When the Prometheus Alertmanager fires a `FrontendHighCPU` or `FrontendErrorRate` incident:
1.  **Industrial Annotation**: The alert payload in GoAlert/Slack contains an autogenerated `trace_link`.
2.  **Instant Pivot**: Clicking this link launches the SRE directly into the Tempo waterfall graph for the exact microservice and time-range identified by the alert.

This eliminates the manual query phase of incident response, enabling machine-speed root cause analysis.

---
*Reference: See `alerting.tf` for the Prometheus rule annotations.*
