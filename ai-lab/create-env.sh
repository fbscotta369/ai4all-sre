...
conda run -n "$ENV_NAME" pip install --no-cache-dir "huggingface_hub<1.0" datasets bitsandbytes sentencepiece protobuf regex pyyaml tqdm safetensors numpy pyarrow psutil scipy pandas
...
conda run -n "$ENV_NAME" pip list | grep -E "torch|unsloth|xformers|triton|transformers|huggingface|datasets"
